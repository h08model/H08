PROGRAM main
  ...
  real, dimension(1000) :: xlon_mat, ylat_mat, lwdo_mat

  call getarg(1, zy)
  call getarg(2, zm)
  call getarg(3, zd)
  
  file1='lwdo_'//zy//'_'//zm//'_'//zd//'.dat'
!  file1=zy//'_'//zm//'_'//zd//'.dat'
  ...
  do while (.true.)
     read(10, *, err=100, end=110) zstation, xlon, ylat, ilwdo
     istation=istation+1

     xlon_mat(istation)=xlon     
     ylat_mat(istation)=ylat
     lwdo_mat(istation)=real(ilwdo)
...
!           write(6, *) istation, distance
           ...
           lwdo_sum=lwdo_sum+lwdo_mat(istation)/(distance**2)
           weight_sum=weight_sum+1.0/(distance**2)

        end do

        lwdo_ave=lwdo_sum/weight_sum
