PROGRAM main
  ...
  real, dimension(1000) :: xlon_mat, ylat_mat, tair_mat ,elev_mat
...
!  file1='tair_'//zy//'_'//zm//'_'//zd//'.dat'
   file1='AMeDAS2A3Q_'//zy//'_'//zm//'_'//zd//'.txt'
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do while (.true.)
     read(10, *, err=100, end=110) zstation, xlon, ylat, itair, ielev
     istation=istation+1

     xlon_mat(istation)=xlon     
     ylat_mat(istation)=ylat
     tair_mat(istation)=real(itair)+273.15
     elev_mat(istation)=real(ielev)
write(6, *) xlon_mat(istation), ylat_mat(istation), tair_mat(istation), elev_mat(istation)
100  continue
  end do
110 continue
