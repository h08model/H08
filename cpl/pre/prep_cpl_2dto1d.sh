...
LLND=67209                  # L for 1d data (land only)
XY="720 360"                       
...
SUFOUT=.hlo                 # Suffix for 1d data (land only)
MAP=.WFDEI
CANSUFIN=.binhlf            # suffix for canal (.bin+SUF)
CANSUFOUT=.binhlo
...
RIVDATVARS="0.5 1.4"
DAMDATVARS="0.0 0.1 0.15 0.5 1.0"
...
for VAR in $INIVARS; do
    ...
    ht2dto1d $LALL $XY $INIIN $LNDMSK $INIOUT
    echo "$INIIN \n$INIOUT"
done
...
for RIVVAR in $RIVDATVARS; do
    ...
    ht2dto1d $LALL $XY $PARAIN $LNDMSK $PARAOUT
    echo "$PARAIN \n$PARAOUT"
done
...
for BINFILE in $BINFILES; do
    ht2dto1d $LALL $XY $BINFILE $LNDMSK tempbin.$BINFILE
    BINOUTFILES=`echo $BINOUTFILES tempbin.$BINFILE` 
done
...
for DATVAR in $DATVARS; do
    ...
    ht2dto1d $LALL $XY $DATIN $LNDMSK $DATOUT
    echo "$DATIN \n$DATOUT"
done
...
for RIVOUTVAR in $RIVOUTVARS; do
	...
	for MONINI in $MONINIS; do
	    ...
	    ht2dto1d $LALL $XY $RIVLRMIN $LNDMSK $RIVLMOUT
	    echo "$RIVSTOIN \n$RIVSTOUT"
	done
	...
	while [ $YEAR -le $YEARMAX ]; do
		...
		ht2dto1d $LALL $XY $RIVLR_IN $LNDMSK $RIVLROUT
		echo "$RIVLR_IN \n$RIVLROUT"
	    done
	    YEAR=`expr $YEAR + 1`
	done
...
for DAMVAR in $DAMVARS; do
	...
	for ALCFILEIN in "${DIRDAM}"/*"${SUFIN}"; do
	    ALCFILEOUT=${ALCFILEIN%${SUFIN}}${SUFOUT}
	    ht2dto1d $LALL $XY $ALCFILEIN $LNDMSK $ALCFILEOUT
	    echo $ALCFILEOUT
	done
    elif [ $DAMVAR = "dam_d2d_" ]; then
	for D2DFILEIN in "${DIRDAM}"/*"${SUFIN}"; do
	    D2DFILEOUT=${D2DFILEIN%${SUFIN}}${SUFOUT}
	    ht2dto1d $LALL $XY $D2DFILEIN $LNDMSK $D2DFILEOUT
	    echo $D2DFILEOUT
	done
    elif [ $DAMVAR = "dam_d2s_" ]; then
	for D2SFILEIN in "${DIRDAM}"/*"${SUFIN}"; do
	    D2SFILEOUT=${D2SFILEIN%${SUFIN}}${SUFOUT}
	    ht2dto1d $LALL $XY $D2SFILEIN $LNDMSK $D2SFILEOUT
	    echo $D2SFILEOUT
	done
    fi
done
...
for FILE in $FILES; do
    ht2dto1d $LALL $XY $FILE $LNDMSK temp.$FILE temp.l2x temp.l2y yes
...
for LDATVAR in $LDATVARS; do
	...
	echo $LDATOUT
	ht2dto1d $LALL $XY $LDATIN $LNDMSK $LDATOUT temp.l2x temp.l2y yes 
